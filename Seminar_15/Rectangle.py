...
class Rectangle:
    ...
    def __init__(self, side_a: int, side_b: int = None):
        ...
        if side_b != int(side_b):
            logging.error(Error_side_b())
            raise Error_side_b()

        if side_a == 0:
            logging.error(Error_side_a_not_null())
            raise Error_side_a_not_null()
        if side_b == 0:
            logging.error(Error_side_b_not_null())
            raise Error_side_b_not_null()

        if side_a < 0:
            logging.error(Error_side_a_negative_number())
            raise Error_side_a_negative_number()
        if side_b < 0:
            logging.error(Error_side_b_negative_number())
            raise Error_side_b_negative_number()

        self.side_a = side_a
        self.side_b = side_b if side_b is not None else side_a

        logging.info("Программа закончила свою работу!!!")
