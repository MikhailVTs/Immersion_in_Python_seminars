...
while True:
    ...
    menu = int(input(f'Пополнить счёт - 1, снять со счёта деньги - 2, выйти - 0: '))
    if menu == 0:
    ...
    if in_your_bank_account > wealth_tax_limit:
        ...
        print(f'Будет произведен вычет на богатство {interest_on_wealth}% в сумме {percent} руб. На Вашем банковском счёте {in_your_bank_account} руб.')
    if menu in (1, 2):
        amount = 1
        while (amount % multiplicity_number) != 0:
            amount = decimal.Decimal(input(f'Введите сумму кратную {multiplicity_number} руб. '))
    if menu == 1:
        counter += 1
